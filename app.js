...
const newDrawingBtn = document.querySelector("div#novo-desenho");
const drawings = document.querySelector("div#drawings");
const header = document.querySelector("header");
const searchBtn = header.querySelector("#pesquisarBtn");
const searchInput = header.querySelector("input#pesquisarQuery");
const more = header.querySelector("#mais");
const configBtn = document.querySelector("#configBtn");
const closeConfigMenuBtn = document.querySelector("#sair-menu-config");
const configMenu = document.querySelector("#menu-configuracao");
...
sharedWorker.port.onmessage = (ev) => {
    ...
    if (msg.type === "create drawing") {
        if (msg.result === "success") {
            ...
            aviso.innerHTML = `<br> ${msg.mensagemErro} <br>`;
            aviso.id = "aviso-input-invalido";
            aviso.setAttribute("role", "alert");
            aviso.style.color = "red";
            inputModal.insertBefore(aviso, inputModal.querySelector("div"));
        }
    }
        ...
        if (msg.result === "success" && msg.section === "tudo" && msg.favoritated) {
            for (let i = 0; i < drawings.children.length; i++) {
                if (drawings.children[i].querySelector(".info").children[0].textContent === msg.name) {
                    drawings.children[i].querySelector(".opcoes").innerHTML = `<svg viewBox='0 0 576 512' height='17.7px' width='19.7px'><path fill='gold' d='M316.9 18C311.6 7 300.4 0 288.1 0s-23.4 7-28.8 18L195 150.3 51.4 171.5c-12 1.8-22 10.2-25.7 21.7s-.7 24.2 7.9 32.7L137.8 329 113.2 474.7c-2 12 3 24.2 12.9 31.3s23 8 33.8 2.3l128.3-68.5 128.3 68.5c10.8 5.7 23.9 4.9 33.8-2.3s14.9-19.3 12.9-31.3L438.5 329 542.7 225.9c8.6-8.5 11.7-21.2 7.9-32.7s-13.7-19.9-25.7-21.7L381.2 150.3 316.9 18z'/></svg> ${drawings[i].children[1].children[1].innerHTML}`;
            ...
            for (let i = 0; i < drawings.children.length; i++) {
                if (drawings.children[i].querySelector(".info").children[0].textContent === msg.name) {
                    drawings.children[i].querySelector(".opcoes").children[0].remove();
...
function renderDrawing(infos) {
    ...
    // primeira opção
    opcoes.querySelectorAll("button")[0].addEventListener('click', function () {
    ...
    for (let i = 0; i < document.querySelectorAll("div#drawings > div.desenho").length; i++) {
        const maisOpcoesContainer = document.querySelectorAll("div#drawings > div.desenho")[i].querySelector("div[popover]");
        maisOpcoesContainer.id = `mais-opcoes-${i + 1}`;

        const maisOpcoesBtn = document.querySelectorAll("div#drawings > div.desenho > div.info")[i].querySelectorAll("button")[1];
        maisOpcoesBtn.setAttribute('popovertarget', maisOpcoesContainer.id);
    }
    return drawing;
}
...
function updateDrawingsMenuPosition() {
    for (const menu of document.querySelectorAll("div.desenho > div[popover]")) {
